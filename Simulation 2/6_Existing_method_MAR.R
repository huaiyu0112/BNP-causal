...
dir.create("6_existing_method_MAR")
...
for (i_rep in rep_seq) {
  ...
  save(ATE_BART, ATE_lm, ATE_IPTW, ATE_TMLE, 
       ...
       file = paste("6_existing_method_MAR/res_", i_rep , "_existing_method_cc",".RData", sep = ""))
...
for (i_rep in rep_seq) {
  ...
  # i_rep = 1
  load( paste("6_existing_method_MAR/res_", i_rep , "_existing_method_cc",".RData", sep = ""))
...
existing_method_res_cc_MS = data.frame(method = method_causal, effect= effect_causal, 
...
write.csv(existing_method_res_cc_MS, file = "6_Existing_method_MAR.csv")
