...
dir.create("5_MICE_plus_existing_method")
...
for (i_rep in rep_seq) {
  ...
  save(true.mi, q.mi, se.mi, check.mi, file = paste("5_MICE_plus_existing_method/res_", i_rep + start_seq, "_mice_causal_multiimp",".RData", sep = ""))
...
for (i_rep in rep_seq) {
  ...
  # i_rep = 1
  load(paste("5_MICE_plus_existing_method/res_", i_rep, "_mice_causal_multiimp",".RData", sep = ""))
...
write.csv(causal_res_MS, file = "5_MICE_plus_existing_method_causal.csv")
...
imp_res = data.frame(Effect= effect_imp, point = est_imp, Emp_sd = emp_sd_imp, RMSE = RMSE_imp, Coverage = coverage_imp, Avg_sd = avg_sd_imp)
write.csv(imp_res, file = "5_MICE_plus_existing_method_imputation.csv")
