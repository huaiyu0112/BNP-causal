...
for (i_rep in rep_seq) {
  ...
  save(ATE_BART, ATE_lm, ATE_IPTW, ATE_TMLE, 
       ...
       file = paste("6_Existing_MAR/res_", i_rep , "_Existing_cc",".RData", sep = ""))
...
for (i_rep in rep_seq) {
  ...
  # i_rep = 1
  load( paste("6_Existing_MAR/res_", i_rep , "_Existing_cc",".RData", sep = ""))
...
Existing_method_res_cc = data.frame(method = method_causal, effect= effect_causal, 
...
write.csv(Existing_method_res_cc, file = "6_Existing_method_MAR.csv")
