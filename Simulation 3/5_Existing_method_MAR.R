...
dir.create("5_Existing_method_est_CC")
...
for (i_rep in rep_seq){
  ...
  save(ATE_Y_BART, ATE_Y_lm, ATE_Y_IPTW, ATE_Y_TMLE, 
       ...
       file = paste("5_Existing_method_est_CC/res_", i_rep , "_Existing_method_cc",".RData", sep = ""))
...
for (i_rep in rep_seq) {
  ...
  # i_rep = 1
  load(paste("5_Existing_method_est_CC/res_", i_rep , "_Existing_method_cc",".RData", sep = ""))
...
write.csv(causal_res_Y, file = "5_Existing_method_est_ATE_Y.csv")
...
causal_res_X = data.frame(method = method_causal, effect= effect_causal, Est= Est_causal, 
                           emp_sd= emp_sd_causal, RMSE = RMSE_causal, MAE = MAE_causal, coverage = coverage_causal, avg_sd = avg_sd_causal)
write.csv(causal_res_X, file = "5_Existing_method_est_ATE_X.csv")
