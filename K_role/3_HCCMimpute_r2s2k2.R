...
save(mu_L3_rep, mu_L4_rep, se_mu_L3_rep, se_mu_L4_rep, IsCovered_mu_L3_rep, IsCovered_mu_L4_rep, file = "HCCMimpute_r2s2k2.Rdata")
...
write.csv(imp_res, file = "HCMM_imputation_r2s2k2.csv")

save.image(file = "HCMM_impute_all_r2s2k2.RData")
...
for (which_imp in seq ){
  ...
  png(file=paste0("check_r2s2k2/Bivariate_plot",which_imp,".png"), width=1200,height=800,pointsize=20)
  plot(L3_orginal, L4_orginal, xlab = "L3", ylab = "L4", col= "grey", pch=16, 
       main = paste("Imputed data", which_imp),  xlim = c(-9, 9), ylim = c(-9, 9), cex= 1, cex.lab=1)
  ...
  # which_imp = 1
  par(mfrow=c(1,1))
  png(file=paste0("check_r2s2k2/Barplot",which_imp,".png"), width=1200,height=800,pointsize=20)
...
par(mfrow=c(1,1))
plot(Z_alloc_trace, type = "l", ylim = c(0, 40), main = "n_comp: k-black, r-red, s-blue ", ylab = "# of occupied comp", xlab = "Iteration")
